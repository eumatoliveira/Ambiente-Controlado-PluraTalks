...
export function CollaboratorShellPage() {
  ...
  return (
    <CollaboratorPageShell title={`Olá, ${state.collaboratorProfile.name.split(' ')[0]}`} description="Acompanhe suas atividades pessoais e comunicações demonstrativas em um só lugar." eyebrow="Visão pessoal">
      ...
      <section className="portal-welcome-card"><div><p className="collaborator-eyebrow">Próxima atividade</p><h2>{pending[0]?.title ?? 'Tudo em dia'}</h2><p>{pending[0]?.description ?? 'Não há avaliações pendentes neste cenário.'}</p></div>{pending[0] ? <Link className="portal-primary-link" to={`/colaborador/responda/${pending[0].id}`}>Começar</Link> : null}</section>
